...
/** ---------- OpenRouter helper through our Cloudflare Function ---------- */
...
function newGameState() {
  return {
    ...
    difficulty: "normal",
    apiStats: {
      connected: false, totalCalls: 0, successfulCalls: 0,
      failedCalls: 0, totalTokensUsed: 0, lastCallTime: null, currentModel: availableModels[0].id,
      lastError: null
    }
  };
}
...
export default function App() {
  ...
  function addLog(result) {
    setG(prev => ({ ...prev, gameLog: [...prev.gameLog, { day: prev.day, event: prev.currentEvent?.title || "Travel", result }] }));
  }
    ...
    <div className="container">
      ...
      <div className="hdr">
        ...
        <div style={{ marginTop: 8, fontSize: 12 }}>
          ...
          <span className="tag">Model: {g.selectedModel}</span>
        </div>
      </div>

      {/* Controls */}
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginBottom: 12 }}>
        <button className="btn" onClick={() => setG(p => ({ ...p, showMap: true }))} title="Map"><MapIcon size={18} /></button>
        <button className="btn green" onClick={() => setG(p => ({ ...p, showShop: true }))} title="Black Market"><ShoppingCart size={18} /></button>
        <button className="btn blue" onClick={() => setG(p => ({ ...p, showSettings: true }))} title="Settings"><Settings size={18} /></button>
        <button className="btn gray" onClick={() => {
          const ng = newGameState();
          setG(ng);
        }} title="New Game"><Upload size={18} /></button>
        <button className="btn orange" onClick={() => {
          const blob = new Blob([JSON.stringify(g, null, 2)], { type: "application/json" });
          const url = URL.createObjectURL(blob);
          const a = document.createElement("a");
          a.href = url; a.download = `modern_trail_${isWin ? "victory" : "run"}_${g.day}days.json`; a.click();
        }} title="Export Save"><Save size={18} /></button>
      </div>
      ...
      <div className="row cols-2" style={{ marginBottom: 16 }}>
        <div className="card">
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            <div className="tag" style={{ marginLeft: "auto" }}>Day {g.day}</div>
          </div>
          <div style={{ fontSize: 14, color: "#aeb6c7", display: "grid", gap: 6 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span>Distance to next</span><span className="mono" style={{ color: "#60a5fa" }}>{g.distanceToNext} miles</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span>Total traveled</span><span className="mono" style={{ color: "#34d399" }}>{g.totalDistance} miles</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span>Progress</span><span style={{ color: "#c084fc" }}>{g.currentLocationIndex}/{g.locations.length - 1}</span>
            </div>
          </div>
        </div>
        <div className="card">
      ...
      {isGameOver ? (
        <div className="card" style={{ textAlign: "center", padding: 24 }}>
          ...
          <div className="row cols-2" style={{ marginTop: 12 }}>
            <button className="btn primary" onClick={() => setG(newGameState())}>New Journey</button>
            <button className="btn blue" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>Back to Top</button>
          </div>
        </div>
      ) : g.currentEvent ? (
        <div className="card">
          ...
          <div className="card" style={{ borderStyle: "solid", borderWidth: 1, borderColor: "#7f1d1d" }}>
            <p style={{ margin: 0, color: "#e5e7eb" }}>{g.currentEvent.description}</p>
          </div>
          <div style={{ display: "grid", gap: 8, marginTop: 8 }}>
            {g.currentEvent.choices.map((c, idx) => (
              <button key={idx} className="btn" style={{ textAlign: "left" }} onClick={() => handleChoice(c)}>
          ...
          {g.isLoading ? (
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
                <button className="btn primary" onClick={generateEvent}>Face the Day</button>
                <button className="btn blue" onClick={advanceDay}>Travel Forward</button>
              </div>
            </>
          )}
        </div>
            ...
            return (
              <div key={i} className="card">
                ...
                <div style={{ display: "grid", gap: 4, fontSize: 14, marginTop: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span>Health:</span><strong style={{ color: m.health <= 30 ? "#f87171" : "#34d399" }}>{m.health}%</strong>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span>Morale:</span><strong style={{ color: m.morale <= 30 ? "#fbbf24" : "#60a5fa" }}>{m.morale}%</strong>
                  </div>
                </div>
                {m.health <= 0 && <div className="tag" style={{ marginTop: 8, borderColor: "#7f1d1d", color: "#fca5a5" }}>Incapacitated</div>}
              </div>
            );
          })}
        </div>
          ...
          <div className="modal">
            ...
            <div className="card" style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                <label>
                  <div style={{ fontSize: 12, color: "#9aa3b2", marginBottom: 4 }}>Model</div>
                  <select
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="btn blue" onClick={testAPIConnection}>Test Connection</button>
                  <button className="btn gray" onClick={() => setG(p => ({ ...p, apiStats: { ...p.apiStats, totalCalls: 0, successfulCalls: 0, failedCalls: 0, totalTokensUsed: 0, lastError: null } }))}>
                    Reset Stats
                  </button>
                </div>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button className="btn" onClick={() => setG(p => ({ ...p, showSettings: false }))}>Close</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="modal">
            ...
            <div className="row cols-2" style={{ marginTop: 10 }}>
              {shopItems.map(item => {
                ...
                return (
                  <div key={item.id} className="card" style={{ opacity: afford ? 1 : 0.6 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
                      ...
                      <button className="btn green" onClick={() => buyItem({ ...item, basePrice: price })} disabled={!afford}>
                        {afford ? "Buy" : "No Cash"}
                      </button>
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12 }}>
              ...
              <button className="btn" onClick={() => setG(p => ({ ...p, showShop: false }))}>Leave Market</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-backdrop">
          <div className="modal">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button className="btn" onClick={() => setG(p => ({ ...p, showMap: false }))}>Close</button>
            </div>
            <div className="card">
              <div style={{ fontWeight: 700, marginBottom: 8, color: "#60a5fa" }}>Journey Progress</div>
              <div className="bar" style={{ marginBottom: 8 }}>
                <div style={{ width: `${progressPct}%`, background: "linear-gradient(90deg,#ef4444,#f59e0b,#10b981)" }} />
              </div>
              <div style={{ display: "grid", gap: 4, fontSize: 14, color: "#cbd5e1" }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span>Current Location:</span><span style={{ color: "#fde68a" }}>{currentLocation}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span>Distance to Next:</span><span style={{ color: "#60a5fa" }}>{g.distanceToNext} miles</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span>Total Distance:</span><span style={{ color: "#34d399" }}>{g.totalDistance} miles</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span>Upcoming:</span><span>{upcoming.join(" • ") || "—"}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {g.lastError && (
        <div className="card" style={{ marginTop: 12, borderColor: "#7f1d1d" }}>
          <strong>Notice:</strong> {g.lastError}
        </div>
      )}
    </div>
  );
